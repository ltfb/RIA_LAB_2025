...
// Keyboard listener for Delete and Backspace keys
document.addEventListener("keydown", function(event) {
    if ((event.key === "Delete" || event.key === "Backspace") && document.activeElement === inputValue) {
...
$('#bhaskaraForm').on('submit', function(e) {
    e.preventDefault();
    ...
    if (a === 0) {
        $('#result').html("The value of 'a' cannot be 0.");
        return;
    }
    ...
    if (disc < 0) {
        $('#result').html("There are no real roots (discriminant < 0).");
    } else if (disc === 0) {
        const x = -b / (2 * a);
        $('#result').html("Double root: x = " + x.toFixed(3));
    } else {
        const x1 = (-b + Math.sqrt(disc)) / (2 * a);
        const x2 = (-b - Math.sqrt(disc)) / (2 * a);
        $('#result').html("x₁ = " + x1.toFixed(3) + "<br>x₂ = " + x2.toFixed(3));
    }
    });
